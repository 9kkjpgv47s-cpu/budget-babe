...
export function PaychecksPanel({
  ...
  return (
    <div className="mt-6 space-y-3 border-t border-zinc-100 pt-6 dark:border-border">
      ...
      {paychecks.length === 0 ? (
        ...
        <ul className="space-y-2">
          {paychecks.map((p) => (
            <li
              key={p.id}
              className="flex flex-wrap items-start justify-between gap-2 rounded-lg border border-border bg-muted/80 px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-900/50"
            >
              <div>
